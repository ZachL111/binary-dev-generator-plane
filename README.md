...
binary-dev-generator-plane is a Rust project for developer tools. It focuses on this technical goal: Build a Rust toolkit that studies generator behavior through deny and allow fixtures, with explainable decision traces and local-only command execution.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 163, risk penalty 5, latency penalty 4, and weight bonus 2. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
